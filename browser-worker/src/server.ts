...
import { PlaywrightInspector, RequestValidationError, type Inspector } from './inspect.js'

const DEFAULT_PORT = 8090
const MAX_REQUEST_BYTES = 64 * 1024
const MAX_CONCURRENT_INSPECTIONS = 2

export function createServer(inspector: Inspector = new PlaywrightInspector()) {
...
async function readJSON(request: IncomingMessage): Promise<unknown> {
  ...
  for await (const chunk of request) {
    ...
    if (size > MAX_REQUEST_BYTES) throw new RequestValidationError('request body exceeds 64 KiB')
    chunks.push(bytes)
  }
  if (size === 0) throw new RequestValidationError('request body is required')
